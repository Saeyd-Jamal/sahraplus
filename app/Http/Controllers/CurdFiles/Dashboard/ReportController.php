...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        $banksId = Account::select('bank_id')->distinct()->pluck('bank_id')->toArray();
        $banks = Bank::select('name')->distinct()->pluck('name')->toArray();
        $month = Carbon::now()->format('Y-m');

        return view('dashboard.pages.report', compact("areas", "working_status", "nature_work", "type_appointment", "field_action", "matrimonial_status", "scientific_qualification", "state_effectiveness", "association", "workplace", "section", "dependence", "establishment", "payroll_statement", 'month', 'banks'));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($request->report_type == 'employees') {
            if ($request->export_type == 'view') {
                ...
                $pdf = PDF::loadView('dashboard.reports.employees', ['employees' =>  $employees, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
        ...
        if ($request->report_type == 'salaries') {
            ...
            $salariesTotal = collect($salaries)->map(function ($salary) use ($month) {
                ...
                if ($fixedEntries == null) {
                    $fixedEntries = $salary->employee->fixedEntries->where('month', '0000-00')->first();
                    if ($fixedEntries != null) {
                        // تحويل الكائن إلى مصفوفة
                        $fixedEntriesArray = $fixedEntries->toArray();

                        // تعديل القيم التي تساوي "-01" إلى "0"
                        foreach ($fixedEntriesArray as $key => $value) {
                            if ($value == "-1") {
                                $fixedEntriesArray[$key] = "0";
                            }
                        }
                        // إذا كنت تريد فقط تعديل الكائن نفسه لاستخدامه
                        $fixedEntries = (object) $fixedEntriesArray;
                    }
                }
                $loans = $salary->employee->loans->where('month', $month)->first();
                return [
                    ...
                    'savings_rate' => $salary->savings_rate ?? '0',
                    'association_loan' => $loans->association_loan ?? '0',
                    'savings_loan' => $loans->savings_loan ?? '0',
                    'shekel_loan' => $loans->shekel_loan ?? '0',
            ...
            $salariesTotalArray = [
                ...
                'savings_rate' => number_format(collect($salariesTotal->pluck('savings_rate')->toArray())->sum(), 2),
            ...
            if ($request->export_type == 'view') {
                $pdf = PDF::loadView(
                    'dashboard.reports.salaries',
                    ['salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $request->month, 'USD' => $USD, 'filter' => $request->all()],
                    [],
                    [
                        'mode' => 'utf-8',
                        'format' => $type_print,
            ...
            if ($request->export_type == 'export_excel') {
                ...
                $salaries = Salary::whereIn('salaries.employee_id', $employees->pluck('id'))
                    ...
                    ->leftJoin('loans', function ($join) use ($month) {
                        $join->on('employees.id', '=', 'loans.employee_id')
                            ->where('loans.month', $month);
                    })
                    ->select(
                        'employees.name',
                        'salaries.month',
                        ...
                        DB::raw('COALESCE(loans.association_loan, 0) as association_loan'),
                        DB::raw('COALESCE(loans.savings_loan, 0) as savings_loan'),
                        DB::raw('COALESCE(loans.shekel_loan, 0) as shekel_loan'),
        ...
        if ($request->report_type == 'accounts') {
            $accounts = Account::whereIn('employee_id', $employees->pluck('id'))->get();
            // معاينة pdf
            if ($request->export_type == 'view') {
                $margin_top = 3;
                if ($request->association != "") {
                ...
                $pdf = PDF::loadView('dashboard.reports.accounts', ['accounts' =>  $accounts, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
            ...
            if ($request->export_type == 'export_excel') {
                ...
                $accounts = Account::whereIn('banks_employees.employee_id', $employees->pluck('id'))
        ...
        if ($request->report_type == 'employees_totals') {
            ...
            if ($request->export_type == 'view') {
                ...
                $pdf = PDF::loadView('dashboard.reports.totals', ['totals' =>  $totals, 'filter' => $request->all(), 'totalsFooterArray' => $totalsFooterArray], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
        ...
        if ($request->report_type == 'employees_fixed') {
            ...
            // معاينة pdf
            if ($request->export_type == 'view') {
                $pdf = PDF::loadView('dashboard.reports.fixed_entries', ['fixed_entries' =>  $fixed_entries, 'filter' => $request->all(), 'month' => $month, 'fixedEntriesTotalsArray' => $fixedEntriesTotalsArray], [], [
                    'mode' => 'utf-8',
                    'format' => $type_print,
                    'default_font_size' => 12,
                    'default_font' => 'Arial',
        ...
        if ($request->report_type == 'bank') {
            ...
            if ($request->export_type == 'view') {
                ...
                $pdf = PDF::loadView('dashboard.reports.bank', ['salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $month, 'monthName' => $monthName, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
        ...
        if ($request->report_type == 'customization') {
            ...
            if ($request->export_type == 'view') {
                $pdf = PDF::loadView(
                    ...
                    ['salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'workplaces' => $workplaces, 'month' => $request->month, 'USD' => $USD, 'filter' => $request->all()],
                    [],
                    [
                        'mode' => 'utf-8',
                        'format' => $type_print,
        ...
        if ($request->report_type == 'employee_form') {
            ...
            if ($request->export_type == 'view' || $request->export_type == 'export_excel') {
                ...
                $pdf = PDF::loadView('dashboard.reports.employee.employee_form', ['employee' =>  $employee, 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'fixedEntries' => $fixedEntries, 'month' => $month, 'year' => $year, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'mode' => 'utf-8',
                    'default_font' => 'Arial',
                    'format' => $type_print,
                    'margin_left' => 3,
            ...
            if ($request->export_type == 'export_pdf') {
                ...
                $pdf = PDF::loadView('dashboard.reports.employee.employee_form', ['employee' =>  $employee, 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $month, 'year' => $year, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'mode' => 'utf-8',
                    'default_font' => 'Arial',
                    'format' => $type_print,
                    'margin_left' => 3,
        ...
        if ($request->report_type == 'employee_accounts') {
            $USD = Currency::where('code', 'USD')->first()->value;
            $month = $request->month ?? Carbon::now()->format('Y-m');
            $to_month = $request->to_month != null && $request->to_month != '' ? Carbon::parse($request->to_month)->format('Y-m') : Carbon::now()->format('Y-m');
            $monthName = $this->monthNameAr[Carbon::parse($month)->format('m')];
            ...
            $exchanges = Exchange::whereIn('employee_id', $employees->pluck('id'))
                ->whereBetween('exchange_date', [$startOfMonth, $endOfMonth])
                ->get();
            // معاينة pdf
            if ($request->export_type == 'view' || $request->export_type == 'export_excel') {
                $margin_top = 3;
                $pdf = PDF::loadView('dashboard.reports.employee.employee_accounts', ['employee' =>  $employees->first(), 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'exchanges' => $exchanges, 'month' => $month, 'to_month' => $to_month, 'months' => $months, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
        ...
        // كشف مستحقات للموظف
        if ($request->report_type == 'employee_totals') {
            $month = $request->month ?? Carbon::now()->format('Y-m');
            ...
            if ($request->export_type == 'view' || $request->export_type == 'export_excel') {
                $margin_top = 3;
                if ($employees->first()->workData->association != "") {
                    $margin_top = 50;
                }
                if ($employees->first()->workData->association == "الكويتي" || $employees->first()->workData->association == "يتيم") {
                    $margin_top = 35;
                }
                $pdf = PDF::loadView('dashboard.reports.employee.employee_receivables', ['employee' =>  $employees->first(), 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $month, 'to_month' => $to_month, 'months' => $months, 'filter' => $request->all(), 'monthName' => $this->monthNameAr], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
        ...
        // كشف مخالصة للموظف
        if ($request->report_type == 'employee_quittance') {
            $USD = Currency::where('code', 'USD')->first()->value;

            $employee = $employees->first();
            $employee->last_salary = $employee->salaries->last() ? $employee->salaries->last()->net_salary : 0;
            $employee->years_service = Numbers::TafqeetNumber($employee->workData->years_service);
            $employee->savings = $employee->totals->total_savings + $employee->totals->total_savings_loan;
            $employee->deductions = $employee->totals->total_association_loan + $employee->totals->total_shekel_loan + $employee->totals->total_savings_loan;
            $employee->health_insurance = $employee->fixedEntries->last() ? $employee->fixedEntries->last()->health_insurance : 0;
            $employee->receivables = ($employee->savings - $employee->deductions);
            $employee->receivables_usd_text = Numbers::TafqeetMoney($employee->receivables, 'USD');
            $employee->receivables_text = Numbers::TafqeetMoney($employee->receivables * $USD, 'ILS');

            // معاينة pdf
            if ($request->export_type == 'view' || $request->export_type == 'export_excel') {
                $margin_top = 3;
                $pdf = PDF::loadView('dashboard.reports.employee.employee_quittance', ['employee' =>  $employee, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
            ...
            if ($request->export_type == 'export_pdf') {
                ...
                $pdf = PDF::loadView('dashboard.reports.employee.employee_quittance', ['employee' =>  $employee, 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'exchanges' => $exchanges, 'month' => $month, 'to_month' => $to_month, 'months' => $months, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
                ...
                return $pdf->download('كشف حساب للموظف' . ' - ' . $employee->name  . ' - ' .  $time . '.pdf');
            }
        }
        // كشف رواتب للموظف
        if ($request->report_type == 'employee_salaries') {
            ...
            if ($request->export_type == 'view' || $request->export_type == 'export_excel') {
                $margin_top = 3;
                $pdf = PDF::loadView('dashboard.reports.employee.employee_salaries', ['employee' =>  $employees->first(), 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $month, 'to_month' => $to_month, 'months' => $months, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
            ...
            if ($request->export_type == 'export_pdf') {
                $margin_top = 3;
                $pdf = PDF::loadView('dashboard.reports.employee.employee_accounts', ['employee' =>  $employees->first(), 'salaries' =>  $salaries, 'salariesTotalArray' => $salariesTotalArray, 'month' => $month, 'to_month' => $to_month, 'months' => $months, 'monthName' => $monthName, 'USD' => $USD, 'filter' => $request->all()], [], [
                    'format' => $type_print,
                    'margin_left' => 3,
                    'margin_right' => 3,
                    'margin_top' => $margin_top,
